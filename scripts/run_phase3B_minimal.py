#!/usr/bin/env python3
"""Phase 3B-minimal — Simple baselines smoke test (source_mean, target_support_mean, monthly_mean, ridge_core).

This is a minimal version for verifying the pipeline and alignment with Phase 3A.
Only 1 seed, reduced K combinations.

Method K settings:
- source_mean_increment: K=0 only (zero-shot baseline)
- target_support_mean_increment: K=4,12,24 (few-shot target mean)
- target_monthly_support_increment: K=12,24 (seasonal mean)
- ridge_core: K=4,12,24 (linear regression baseline)
"""
...
def fit_ridge_core(target_region, K, splits_by_key, region_mask_int):
    """Fit Ridge regression predictor (core feature set, K>0)."""
...
def evaluate_on_query(predictor, method_name, target_region, K, seed, splits_by_key, region_mask_int, result_list):
    ...
    experiment_id = f"phase3B_minimal_{method_name}_{target_region}_K{K}_S{seed}_target_query"
...
def run_phase3B_minimal() -> None:
    ...
    # Total evaluations:
    # source_mean_increment: 6 regions * 1 K (K=0) * 1 seed = 6
    # target_support_mean_increment: 6 * 3 K (4,12,24) * 1 seed = 18
    # target_monthly_support_increment: 6 * 2 K (12,24) * 1 seed = 12
    # ridge_core: 6 * 3 K (4,12,24) * 1 seed = 18
    # Total = 54 evaluations
    TOTAL_EVALS = 54
    print(f"\nPhase 3B-minimal: {TOTAL_EVALS} evaluations (smoke test)", flush=True)
    t_start = time.time()
    ...
    for target_region in REGIONS:
        print(f"\nProcessing region {target_region}...", flush=True)
        # Fit source_mean for K=0
        K = 0
        cache_key = (target_region, K)
        if cache_key not in source_mean_cache:
            print(f"  Fitting source_mean {target_region} K={K}...", flush=True)
            t0 = time.time()
        ...
        # Fit target_mean for K=4,12,24
        for K in _TARGET_MEAN_K:
            cache_key = (target_region, K)
            if cache_key not in target_mean_cache:
                target_mean_cache[cache_key] = fit_target_mean(target_region, K, splits_by_key, region_mask_int)

        # Fit monthly_mean for K=12,24
        for K in _MONTHLY_MEAN_K:
            cache_key = (target_region, K)
            if cache_key not in monthly_mean_cache:
                monthly_mean_cache[cache_key] = fit_monthly_mean(target_region, K, splits_by_key, region_mask_int)

        # Fit ridge_core for K=4,12,24
        for K in _RIDGE_CORE_K:
            ridge_key = (target_region, K)
            if ridge_key not in ridge_cache:
                ridge_cache[ridge_key] = fit_ridge_core(target_region, K, splits_by_key, region_mask_int)

        # Evaluate for the single seed
        for seed in SEEDS:
            # source_mean K=0
            K = 0
            print(f"  Evaluating {target_region} K={K} seed={seed} source_mean...", flush=True)
            ...
            print(f"    -> eval_count now {eval_count+1}", flush=True)
            eval_count += 1

            # target_mean K=4,12,24
            for K in _TARGET_MEAN_K:
                predictor_target = target_mean_cache.get((target_region, K))
            ...
            # monthly_mean K=12,24
            for K in _MONTHLY_MEAN_K:
                predictor_monthly = monthly_mean_cache.get((target_region, K))
                if predictor_monthly is not None:
                    evaluate_on_query(predictor_monthly, "target_monthly_support_increment",
                                     target_region, K, seed, splits_by_key, region_mask_int,
                                     all_results["target_monthly_support_increment"])
                    eval_count += 1

            # ridge_core K=4,12,24
            for K in _RIDGE_CORE_K:
                predictor_ridge = ridge_cache.get((target_region, K))
                if predictor_ridge is not None:
                    evaluate_on_query(predictor_ridge, "ridge_core",
                                     target_region, K, seed, splits_by_key, region_mask_int,
        ...
        remaining = (TOTAL_EVALS - eval_count) / rate if rate > 0 else 0
        if eval_count % 10 == 0 or eval_count == TOTAL_EVALS:
            sys.stdout.write(f"  [{eval_count}/{TOTAL_EVALS}] elapsed={elapsed:.0f}s ETA={remaining:.0f}s\n")
            sys.stdout.flush()
    ...
    for method_name, results in all_results.items():
        ...
        out_path = OUTPUT_DIR_BASE / f"phase3B_minimal_{method_name}_US" / "metrics_long.csv"
