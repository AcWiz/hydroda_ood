#!/usr/bin/env python3
"""Phase 3B — Simple baselines evaluation, 1-seed reference run.
...
108 evaluations total (6 regions × 4 K × 1 seed × 7 methods - K=0 only has source_mean).
"""
...
def run_phase3B() -> None:
    ...
    # Total evaluations (1-seed):
    # source_mean: 6*4*1=24, target_mean: 6*3*1=18,
    # monthly_mean: 6*2*1=12, ridge_core/input_full/input_geo_full: 6*3*1*3=54
    # Total = 108
    print(f"\nPhase 3B: 108 evaluations (1-seed reference)", flush=True)
    t_start = time.time()
    ...
    for target_region in REGIONS:
        for K in K_VALUES:
            ...
            for seed in SEEDS:
                ...
                remaining = (108 - eval_count) / rate if rate > 0 else 0
                if eval_count % 20 == 0 or eval_count == 108:
                    elapsed = time.time() - t_start
                    rate = eval_count / elapsed if elapsed > 0 else 0
                    remaining = (108 - eval_count) / rate if rate > 0 else 0
                    _progress = f"  [{eval_count}/108] elapsed={elapsed:.0f}s ETA={remaining:.0f}s\n"
