#!/usr/bin/env python3
"""M3_13 physics-gain guarded HyperDA-TRUST runner."""
...
class SourceBasePathPredictor(PromptConditionedBackbonePredictor):
    """Expose the frozen source-base path of an M3_1 HyperDA checkpoint."""

    method_name = "m3_1_frozen_source_base_path"

    def __init__(self, *args: Any, **kwargs: Any) -> None:
        super().__init__(*args, **kwargs)
        self.method_name = "m3_1_frozen_source_base_path"

    def predict(self, sample: Mapping[str, Any]) -> dict[str, np.ndarray]:
        if not isinstance(self.model, HyperAdapterConditionalResUNet):
            raise ValueError("M3_13 source-base path requires a HyperAdapterConditionalResUNet checkpoint")
        x = torch.from_numpy(np.asarray(sample["x"], dtype=np.float32)).unsqueeze(0).to(self.device)
        x_norm = self._normalize(x)
        with torch.no_grad():
            pred = self.model.source_base_forward(x_norm)
        ...
        if self._has_inc_norm:
            pred_inc_s = (pred_inc_s * self._inc_std[0] + self._inc_mean[0]).astype(np.float32)
            pred_inc_r = (pred_inc_r * self._inc_std[1] + self._inc_mean[1]).astype(np.float32)
        if self.apply_residual_gain:
            alpha_s = float(self.alpha_surface)
            alpha_r = float(self.alpha_rootzone)
        else:
            alpha_s = 1.0
            alpha_r = 1.0
        ...
        if self.apply_support_affine_calibration:
            ...
            def coeff(variable: str) -> tuple[float, float]:
            ...
            a_s, b_s = coeff("surface")
            a_r, b_r = coeff("rootzone")
...
def _source_trust_neighbors_from_predictor(
    predictor: PromptConditionedBackbonePredictor,
) -> list[dict[str, Any]]:
    bank = getattr(predictor, "_source_trust_bank_state", None)
    model = getattr(predictor, "model", None)
    if not isinstance(bank, Mapping) or model is None:
        return []
    metadata = bank.get("source_region_month_metadata")
    if not isinstance(metadata, list):
        return []
    routing = getattr(model, "last_trust_routing_summary", {}) or {}
    if not isinstance(routing, Mapping):
        return []
    summary = routing.get("bottleneck") or next(iter(routing.values()), {})
    if not isinstance(summary, Mapping):
        return []
    indices = summary.get("nearest_neighbor_indices") or []
    if not indices:
        return []
    if isinstance(indices[0], list):
        indices = indices[0]
    temperature = max(float(summary.get("distance_temperature", bank.get("distance_temperature", 1.0)) or 1.0), 1e-6)
    distances = summary.get("nearest_neighbor_distances") or []
    ...
    for rank, raw_index in enumerate(indices):
        ...
        if not isinstance(meta, Mapping):
            continue
        region = meta.get("source_region_id") or meta.get("source_region") or meta.get("sample_region_id")
        if not region and "source_region_compact_id" in meta:
            compact_id = int(meta["source_region_compact_id"])
            source_regions = list(getattr(predictor, "source_regions", []) or [])
            region = source_regions[compact_id] if compact_id < len(source_regions) else f"US-R{compact_id + 1}"
        month = int(meta.get("month", 1))
        if month < 1 or month > 12 or not region:
            continue
        distance = None
        if isinstance(distances, list) and rank < len(distances):
            ...
            if row is not None:
                distance = float(row)
        weight = float(np.exp(-float(distance) / temperature)) if distance is not None else 1.0 / float(rank + 1)
        neighbors.append(
            {
...
def _build_predictor(
    *,
    ...
    seed: int,
    splits_json: str,
    source_base_path: bool = False,
) -> PromptConditionedBackbonePredictor:
    cls = SourceBasePathPredictor if source_base_path else PromptConditionedBackbonePredictor
    predictor = cls(
...
def _iter_source_records(
    *,
    ...
    if include_predictions:
        ...
        source_base_predictor = _build_predictor(
            checkpoint=checkpoint,
            device=device,
            target_region=target_region,
            target_context_prompt=False,
        ...
        source_regions = list(getattr(base_predictor, "source_regions", []) or [])
        ...
        for source_region in source_regions:
            ...
            for idx in range(n):
                ...
                if base_predictor is not None:
                    ...
                    neighbors = _source_trust_neighbors_from_predictor(base_predictor)
                    if neighbors:
                        record["source_trust_neighbors"] = neighbors
                if source_base_predictor is not None:
                    source_pred = source_base_predictor.predict(sample)
                    record["source_base_increment_surface"] = source_pred["pred_increment_surface"]
                    record["source_base_increment_rootzone"] = source_pred["pred_increment_rootzone"]
                yield record
    finally:
        dataset.close()
...
def cmd_build_bank(args: argparse.Namespace) -> None:
    ...
    try:
        for source_region in _source_regions_for_bank(args.target_region):
            ...
            for idx in range(n):
                accumulator.update(dataset[idx])
...
def cmd_write_gate_report(args: argparse.Namespace) -> None:
    ...
    report = {
        ...
        "source_gain_bank_hash": selection.get("source_gain_bank_hash", ""),
...
def _selection_from_args(args: argparse.Namespace) -> tuple[float, float, dict[str, Any]]:
    if args.source_gate:
        ...
        if gate.get("schema_version") != "m3_13_source_gate_report_v1":
            raise ValueError(f"Unsupported M3_13 source gate report: {gate.get('schema_version')!r}")
        if gate.get("method_id") != PHYS_GAIN_GUARD_METHOD_ID:
            raise ValueError("M3_13 source gate report method_id mismatch")
        if args.split_type in {"target_eval", "target_query"} and not bool(gate.get("target_eval_allowed", False)):
            raise ValueError("M3_13 target_eval refused: source_gate.json does not allow target_eval")
    if args.selection:
        selection = json.loads(Path(args.selection).read_text(encoding="utf-8"))
        validate_router_metadata_no_target_selection(selection)
    ...
    if args.split_type in {"target_eval", "target_query"}:
        raise ValueError("M3_13 target_eval requires --selection and --source_gate from source_val")
...
def _write_experiment_card(
    *,
    ...
    out = Path(path)
    out.parent.mkdir(parents=True, exist_ok=True)
    text = f"""# M3_13 Physics-Gain Guarded HyperDA-TRUST Experiment Card
...
- Source gate: {summary.get('source_gate_path')}
...
- Guard action: shrink-only around source-base residual; no new residual and no amplification
...
def cmd_evaluate(args: argparse.Namespace) -> None:
    ...
    predictor = PhysGainGuardedHyperTrustPredictor(
        base_predictor,
        source_base_predictor,
        bank,
    ...
    try:
        rows, hashes = evaluate_split(
            ...
            experiment_id=f"{PHYS_GAIN_GUARD_METHOD_ID}_{args.target_region}_K{args.K}_S{args.seed}",
    ...
    summary = {
        ...
        "guard_action": "source_base + guard * (pred_M3_1 - source_base)",
    ...
    if args.write_experiment_card:
        _write_experiment_card(
            ...
            summary=summary,
            command_hint=f"bash run/m3_13_phys_gain_guarded_hypertrust.sh {args.split_type} {args.target_region} {args.seed}",
        )
    print(
        json.dumps(
            {
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="M3_13 physics-gain guarded HyperDA-TRUST")
    ...
    build = sub.add_parser("build-bank", help="Build source_fit physics-gain guard bank")
    add_common(build)
    build.add_argument("--bank_out", required=True)
    build.add_argument("--ridge_lambda", type=float, default=1e-4)
    ...
    select = sub.add_parser("select-eta", help="Select guard eta pair on source_val")
    add_common(select)
    select.add_argument("--bank", required=True)
    select.add_argument("--selection_out", required=True)
    select.add_argument("--eta_grid", default="0,0.02,0.05,0.10")
    select.add_argument("--guard_min", type=float, default=0.90)
    ...
    evaluate = sub.add_parser("evaluate", help="Evaluate M3_13 guarded router")
    add_common(evaluate)
    ...
    evaluate.add_argument("--guard_min", type=float, default=0.90)
    evaluate.add_argument("--proposal_clip_scale", type=float, default=1.0)
    evaluate.add_argument("--split_type", default="target_eval")
    evaluate.add_argument("--output_dir", default="artifacts/runs/M3_13_phys_gain_guarded_hypertrust/eval")
