#!/bin/bash
# Phase 4B Prompt-Conditioned Inference: source_val vs target_query
#
# Usage:
#   bash run/phase4_prompt_conditioned_inference.sh
#   bash run/phase4_prompt_conditioned_inference.sh /path/to/checkpoint.pt
#
# Evaluates the prompt-conditioned shared backbone on:
#   - source_val: 2021 held-out source regions R2-R6 (in-domain, NOT target)
...
# Parse + print comparison table (same format as source-only inference)
echo ""
echo "============================================"
...
output_base = Path("artifacts/results/phase4_prompt_conditioned_inference").absolute()
...
print(f"Method:      prompt_conditioned_shared_backbone")
...
if src and tgt:
    ...
    print()
    print("> Note: positive gap = target better than expected; negative = OOD degradation.")
