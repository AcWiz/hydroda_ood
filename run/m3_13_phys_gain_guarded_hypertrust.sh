...
# M3_13: source-calibrated physics-gain no-harm guard around a frozen
# M3_1 HyperDA-TRUST anchor. The guard can only shrink
# pred_M3_1 - source_base and never adds a new output residual.
#
# Usage:
#   bash run/m3_13_phys_gain_guarded_hypertrust.sh build-bank US-R2 0 0
#   bash run/m3_13_phys_gain_guarded_hypertrust.sh select-eta US-R2 0 0
#   bash run/m3_13_phys_gain_guarded_hypertrust.sh eval-source US-R2 0 0
#   bash run/m3_13_phys_gain_guarded_hypertrust.sh eval-target US-R2 0 0
#   bash run/m3_13_phys_gain_guarded_hypertrust.sh all US-R2 0 0
#
...
if [[ -z "${SOURCE_CHECKPOINT}" || ! -f "${SOURCE_CHECKPOINT}" ]]; then
    echo "ERROR: M3_13 requires an M3_1 anchor checkpoint for ${TARGET_REGION} seed ${SEED}." >&2
...
OUTPUT_BASE="${OUTPUT_BASE:-artifacts/runs/M3_13_phys_gain_guarded_hypertrust/${TARGET_REGION}_s${SEED}_${DEFAULT_SAMPLE_TAG}}"
BANK_PATH="${BANK_PATH:-${OUTPUT_BASE}/source_fit_phys_gain_guard_bank.json}"
...
ETA_GRID="${ETA_GRID:-0,0.02,0.05,0.10}"
GUARD_MIN="${GUARD_MIN:-0.90}"
...
build_bank() {
  python scripts/eval/m3_13_phys_gain_guarded_hypertrust.py build-bank \
    "${COMMON_ARGS[@]}" \
    --bank_out "${BANK_PATH}" \
...
select_eta() {
  python scripts/eval/m3_13_phys_gain_guarded_hypertrust.py select-eta \
    "${COMMON_ARGS[@]}" \
    --bank "${BANK_PATH}" \
...
write_gate_report() {
  python scripts/eval/m3_13_phys_gain_guarded_hypertrust.py write-gate-report \
    --target_region "${TARGET_REGION}" \
    --K 0 \
    --seed "${SEED}" \
...
eval_split() {
  ...
  local out_dir="$2"
  shift 2
  python scripts/eval/m3_13_phys_gain_guarded_hypertrust.py evaluate \
    "${COMMON_ARGS[@]}" \
    --bank "${BANK_PATH}" \
    --selection "${SELECTION_PATH}" \
...
ensure_bank() {
  if [[ "${FORCE_REBUILD}" == "1" || ! -f "${BANK_PATH}" ]]; then
    ...
    echo "Reusing M3_13 physics-gain guard bank cache: ${BANK_PATH}"
  fi
}
...
ensure_selection() {
  ...
  if [[ "${FORCE_REBUILD}" == "1" || ! -f "${SELECTION_PATH}" ]]; then
    ...
    echo "Reusing M3_13 eta selection cache: ${SELECTION_PATH}"
  fi
  write_gate_report
}
...
ok = (
    gate.get("schema_version") == "m3_13_source_gate_report_v1"
    and gate.get("method_id") == "M3_13_phys_gain_guarded_hypertrust"
...
case "${MODE}" in
  ...
  eval-target)
    ...
    if ! source_gate_passes; then
      echo "ERROR: M3_13 source gate did not pass; refusing target_eval." >&2
  ...
  all)
    ...
    if source_gate_passes; then
      ...
      echo "M3_13 source gate did not pass; target_eval intentionally not run."
...
echo "M3_13 artifacts: ${OUTPUT_BASE}"
