#!/bin/bash
# Phase 5: Hydro-MSR-ROSE target historical adaptation entrypoint.
#
# ROSE = Robust Observation-Space Encoding. This wrapper keeps the frozen
# HyperDA source prior unchanged and swaps only the target spatial residual
# adapter to a Hydro-MSR adapter augmented with bounded DA-aware input-side
...
#
# Usage:
#   bash run/phase5_hydro_msr_rose.sh
#   bash run/phase5_hydro_msr_rose.sh <source_checkpoint> US-R1 0 1
#   ENABLE_HYDRO_MSR_DA_FILM=1 bash run/phase5_hydro_msr_rose.sh <source_checkpoint> US-R1 0 1
...
if [[ -z "${SOURCE_CHECKPOINT}" || ! -f "${SOURCE_CHECKPOINT}" ]]; then
    ...
    echo "  bash run/phase5_hydro_msr_rose.sh <source_checkpoint> ${TARGET_REGION} ${SEED} ${CUDA_VISIBLE_DEVICES}" >&2
    exit 2
fi
...
TARGET_SPATIAL_REFINE_TYPE="${TARGET_SPATIAL_REFINE_TYPE:-hydro_msr_rose}"
...
RUN_NAME="${RUN_NAME:-phase5_hydro_msr_rose_${TARGET_REGION}_s${SEED}}"
RESUME_ARGS=()
HAS_RESUME_ARG=0
...
echo "============================================"
echo "Phase 5 Hydro-MSR-ROSE Target Historical Adaptation"
...
echo "  rose=input-side robust observation-space encoding"
