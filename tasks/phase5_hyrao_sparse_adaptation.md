# Superseded Phase 5 Entry

This file is retained only for compatibility with old command names. HyRAO and
sparse adaptation are not active method names, paper-main baselines, or current
protocol surfaces.

Use the active Phase 5 task instead:

```text
tasks/phase5_hyperda_safe_zero_few_shot.md
```

Active protocol:

```text
V4.4 zero/few-shot target generalization
HyperDA-TRUST K=0 mainline; SAFE K-shot diagnostic / future extension
K in {0,4,12}
target_context=2015-2021 input-side only
target_support=K labeled cycles
target_val=unused_in_main_protocol
target_eval=2023-2025 final offline evaluation only
```
