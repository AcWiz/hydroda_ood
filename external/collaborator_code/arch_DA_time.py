...
class SwinBlock(nn.Module):
    ...
    def forward(self, x):
        # Removed adaLN_modulation and time embedding
...
class Sformer(nn.Module):
    def __init__(
        ...
        self.default_vars = [
            'Wind_f_inst', 'Rainf_f_tavg', 'Tair_f_inst', 'Qair_f_inst', 'Psurf_f_inst',
            'SWdown_f_tavg', 'LWdown_f_tavg', 'SoilTMP0_10cm_inst', 'SoilMoi0_10cm_inst'
        ]
    ...
    def forward_encoder(self, x: torch.Tensor, time_code=None, noise = None):
        # x: [B, self.num_vars, H, W] shape (self.num_vars variables, each with 1 layer)
        # Split input into self.num_vars variables
        var1, var2, var3, var4, var5, var6 = x.split(1, dim = 1)  # Each: [B, 1, H, W]

        # Apply separate patch embedding for each variable
        in_1 = self.patch_embed_1(var1)  # [B, embed_dim, H/p, W/p]
        ...
        h = torch.concat([in_1, in_2, in_3, in_4, in_5, in_6], dim = 1)  # [B, self.num_vars*embed_dim, H/p, W/p]
        h = h.flatten(2).transpose(1, 2)  # [B, HW/p^2, self.num_vars*embed_dim]
        h = self.in_norm(h)
    ...
    def forward(self, x, time_code=None, noise=None):
        ...
        Args:
            x: [B, 8, H, W] shape. Input weather/climate variables (11 variables, each with 1 layer)
            variables: List of input variable names
            noise: Optional noise for robustness

        Returns:
            preds: [B, 4, H, W] shape
        """
        mask = x[:,-1:]            
        out_transformers = self.forward_encoder(x, time_code=time_code, noise=noise)  # [B, L, self.num_vars*embed_dim]
        preds = self.head(out_transformers)  # [B, L, p*p]
        preds = self.unpatchify(preds)  # [B, 1, H, W]
        preds = preds * mask
        preds = preds + x[:, :2]  # Residual connection
