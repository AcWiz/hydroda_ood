...
class Sformer(nn.Module):
    ...
    def forward(self, x, noise = None):
        ...
        Args:
            x: [B, 12, H, W] shape. Input weather/climate variables (11 variables, each with 1 layer)
        ...
        Returns:
            preds: [B, 6, H, W] shape
        """
        mean_le, std_le = 31.187826, 41.93185
        mean_sh, std_sh = 66.21313, 132.90985
        scale_factor = torch.tensor([(0-mean_le)/std_le, (0-mean_sh)/std_sh], device=x.device).view(1, 2, 1, 1)
        mask = x[:,-1:]
        x = x[:,:-1]  # Remove the mask variable

        out_transformers = self.forward_encoder(x,
                                                noise)  # [B, L, self.num_vars*embed_dim]
        preds = self.head(out_transformers)  # [B, L, p*p]
        preds = self.unpatchify(preds)  # [B, 2, H, W]
        preds = preds * mask + scale_factor * (1 - mask)
